...
## Selective LoRA with `lora_target_regex`
...
To optimize memory usage, you can use the `--lora-target-regex` command-line parameter to selectively initialize only the required LoRA layers.
...
By using `--lora-target-regex` to target only the self-attention layers, you can avoid allocating memory for the FFN/MoE LoRA layers, saving over 2.4 GiB of VRAM in this specific example.

```bash
--lora-target-regex '.*\\.self_attn\\..*'
```

This command ensures that only modules whose names contain `.self_attn.` will have LoRA layers initialized.
